...
def test_insta4288db_destroy():
    """Verify insta4288db destroy removes DB file."""
    assert_is_shell_script("bin/insta4288db")
    subprocess.run(["bin/insta4288db", "destroy"], check=True)
    assert not os.path.exists("var/insta4288.sqlite3")
    assert not os.path.exists("var/uploads")


def test_insta4288db_create():
    """Verify insta4288db create populates DB with default data."""
    assert_is_shell_script("bin/insta4288db")

    # Destroy, then create database
    subprocess.run(["bin/insta4288db", "destroy"], check=True)
    subprocess.run(["bin/insta4288db", "create"], check=True)

    # Verify files were created
    assert os.path.exists("var/insta4288.sqlite3")
    ...
    connection = sqlite3.connect("var/insta4288.sqlite3")
...
def test_insta4288db_reset():
    """Verify insta4288db reset does a destroy and a create."""
    # Create a "stale" database file
    dbfile = pathlib.Path("var/insta4288.sqlite3")
    dbfile.write_text("this should be overwritten")

    # Reset the database
    subprocess.run(["bin/insta4288db", "reset"], check=True)
...
def test_insta4288db_dump():
    """Spot check insta4288db dump for a few data points."""
    assert_is_shell_script("bin/insta4288db")
    subprocess.run(["bin/insta4288db", "reset"], check=True)
    output = subprocess.run(
        ["bin/insta4288db", "dump"],
...
def test_insta4288run(setup_teardown):
    """Verify insta4288run script behavior."""
    ...
    db_path = pathlib.Path("var/insta4288.sqlite3")
    if db_path.exists():
        db_path.unlink()
    completed_process = subprocess.run(["bin/insta4288run"])
    assert completed_process.returncode != 0

    # Create database
    completed_process = subprocess.run(["bin/insta4288db", "create"])
    assert os.path.exists("var/insta4288.sqlite3")

    # Execute student run script in a concurrent thread.  Don't check the
    # return code because test cleanup will kill the process
    assert_is_shell_script("bin/insta4288run")
    thread = threading.Thread(
        target=subprocess.run,
        args=(["bin/insta4288run"],),
        kwargs={"check": False},
    )
    thread.start()
...
def test_insta4288test():
    """Verify insta4288test script contains correct commands."""
    assert_is_shell_script("bin/insta4288test")
    lines = pathlib.Path("bin/insta4288test")\
